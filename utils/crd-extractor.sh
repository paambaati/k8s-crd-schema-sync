...
# NOTE: Ported from https://github.com/datreeio/CRDs-catalog/blob/main/Utilities/crd-extractor.sh
# and modified to use `uv` instead of Python.
...
if ! command -v uv &>/dev/null; then
    printf "uv is required for this utility, and is not installed on your machine"
    printf "please visit https://docs.astral.sh/uv/getting-started/ to install it"
    exit 1
fi
...
cd "$CONVERSION_TMP"
uv run --native-tls --with pyyaml python3 "$CONVERTER_SCRIPT" "$TMP"/*.yaml
conversionResult=$?
...
if [ ${#NEW_SCHEMAS[@]} -gt 0 ]; then
    ...
    if [ -z "${OUTPUT_DIR:-}" ]; then
        ...
        for crdGroup in "${!PROCESSED_GROUPS[@]}"; do
            ...
            if [ -d "$groupDir" ]; then
                # Copy to schemas directory in project root
                repoDir="$(dirname "$SCRIPT_DIR")/schemas/$crdGroup"
                mkdir -p "$repoDir"
